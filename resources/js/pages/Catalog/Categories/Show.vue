...
import type { Category, Course, Paginated } from '@/types/project';
...
// Structured data para la categoría
const categorySchema = computed(() =>
    generateStructuredData('CollectionPage', {
        name: props.category.title,
        description: props.category.description || `Cursos de ${props.category.title}`,
        url: page.url,
    }),
);
...
const breadcrumbs: BreadcrumbItemType[] = [
    ...
    },
    {
        ...
        href: '/categorias/' + props.category.slug,
    },
];
                                ...
                                <div class="results-info-premium">
                                    ...
                                    <div class="info-text">
                                        <span class="info-count"
                                            ><strong>{{ courses.from }}</strong> de <strong>{{ courses.total }}</strong> materiales de estudio</span
                                        >
                                    </div>
                                </div>
                            </div>
...
/* Selector de Filtro Personalizado - Dentro de la misma caja */
.filter-selector-row {
    width: 100%;
    flex-basis: 100%;
    padding-top: 24px;
...
.custom-filter-select {
    ...
    background-image: url("data:image/svg+xml,%3Csvg width='16' height='10' viewBox='0 0 16 10' fill='none' xmlns='http://www.w3.org/2000/svg'%3E%3Cpath d='M1 1.5L8 8.5L15 1.5' stroke='%23133a54' stroke-width='3' stroke-linecap='round' stroke-linejoin='round'/%3E%3C/svg%3E");
...
.custom-filter-select:hover {
    color: #133a54;
    background-color: rgba(19, 58, 84, 0.06);
...
/* Información de ayuda debajo del selector */
.opec-help-info {
    display: flex;
    align-items: center;
    gap: 10px;
...
.opec-help-info a:hover {
    color: #0d2a3e;
    text-decoration: none;
}

/* Eliminar estilos antiguos */
.filter-selector-premium,
.filter-selector-full-width {
    display: none;
}
